...
class Cache:
    ...
    def __init__(self, edge_cache_ratio: int, node_cache_ratio: int,
        ...
        self.node_cache_overhead = 0
        self.edge_cache_overhead = 0

        self.node_feat_fetch_time = 0
        self.edge_feat_fetch_time = 0
        ...
        if self.dim_node_feat != 0:
            ...
            if self.node_cache_ratio == 1.0:
                self.cache_node_flag[:] = True
                self.cache_node_map[:] = torch.arange(
                    num_nodes, dtype=torch.int64, device=self.device)
                self.cache_index_to_node_id[:] = torch.arange(
                    self.node_capacity, dtype=torch.int64, device=self.device)
                self.cache_node_buffer[:] = node_feats.to(self.device)
        ...
        if self.dim_edge_feat != 0:
            ...
            if self.edge_cache_ratio == 1.0:
                self.cache_edge_flag[:] = True
                self.cache_edge_map[:] = torch.arange(
                    num_edges, dtype=torch.int64, device=self.device)
                self.cache_index_to_edge_id[:] = torch.arange(
                    self.edge_capacity, dtype=torch.int64, device=self.device)
                self.cache_edge_buffer[:] = edge_feats.to(self.device)
    ...
    def init_cache(self, *args, **kwargs):
        ...
        if self.distributed:
            ...
            if self.dim_edge_feat != 0 and self.cache_edge_ratio < 1:
                cache_edge_id = torch.arange(
                    self.edge_capacity, dtype=torch.int64, device=self.device)
        ...
        self.edge_cache_overhead = time.time() - start
    ...
    def fetch_feature(self, mfgs: List[List[DGLBlock]],
                      ...
                      target_edge_features: bool = False):
        ...
        if self.dim_node_feat != 0:
            ...
            for b in mfgs[0]:
                ...
                if self.node_cache_ratio == 1.0:
                    hit_ratio_sum += 1
                    i += 1
                    b.srcdata['h'] = self.cache_node_buffer[nodes].float()
                    continue
                cache_mask = self.cache_node_flag[nodes]

                hit_ratio = torch.sum(cache_mask).item() / len(nodes)
                ...
                cached_nodes = nodes[cache_mask]
                cached_node_index = self.cache_node_map[cached_nodes]
                node_feature[cache_mask] = self.cache_node_buffer[cached_node_index]
                # fetch the uncached features
                uncached_mask = ~cache_mask
                uncached_sum = uncached_mask.sum().item()

                node_feat_start_fetch_time = time.time()
                if uncached_sum > 0:
                    ...
                    if self.distributed:
                        ...
                        if self.pinned_nfeat_buffs is not None:
                            ...
                            uncached_node_feature = self.node_feats[uncached_node_id_unique.to('cpu')].to(
                                self.device, non_blocking=True)
                ...
                self.node_feat_fetch_time += time.time() - node_feat_start_fetch_time

                if update_cache and self.node_cache_ratio > 0 and uncached_sum > 0:
                    start = time.time()
                    if self.name == 'predictive_cache' or self.name == 'ema_cache':
                        ...
                        self.update_node_cache(cached_node_index=cached_node_index,
                                               ...
                                               all_nodes=all_nodes, cached_nodes=cached_nodes)
                    else:
                        self.update_node_cache(cached_node_index=cached_node_index,
                                               uncached_node_id=uncached_node_id_unique,
                                               uncached_node_feature=uncached_node_feature)
                    self.node_cache_overhead += time.time() - start
        ...
        if self.dim_edge_feat != 0:
            ...
            for mfg in mfgs:
                for b in mfg:
                    ...
                    if self.edge_cache_ratio == 1.0:
                        hit_ratio_sum += 1
                        i += 1
                        b.edata['f'] = self.cache_edge_buffer[edges].float()
                        continue

                    cache_mask = self.cache_edge_flag[edges]
                    hit_ratio = torch.sum(cache_mask).item() / len(edges)
                    hit_ratio_sum += hit_ratio
                    ...
                    # fetch the cached features
                    cached_edges = edges[cache_mask]
                    cached_edge_index = self.cache_edge_map[cached_edges]
                    edge_feature[cache_mask] = self.cache_edge_buffer[cached_edge_index]
                    # fetch the uncached features
                    uncached_mask = ~cache_mask
                    uncached_edge_id = edges[uncached_mask]

                    edge_feat_start_fetch_time = time.time()
                    ...
                    self.edge_feat_fetch_time += time.time() - edge_feat_start_fetch_time

                    if update_cache and self.edge_cache_ratio > 0 and len(uncached_edge_id) > 0:
                        start = time.time()
                        if self.name == 'predictive_cache' or self.name == 'ema_cache':
                            ...
                            self.update_edge_cache(cached_edge_index=cached_edge_index,
                                               ...
                                               all_edges=all_edges, cached_edges=cached_edges)
                        else:
                            self.update_edge_cache(cached_edge_index=cached_edge_index,
                                                   uncached_edge_id=uncached_edge_id_unique,
                                                   uncached_edge_feature=uncached_edge_feature)
            ...
            if target_edge_features:
                if self.distributed:
                    ...
                    if self.cache_edge_ratio < 1:
                        self.target_edge_features = self.edge_feats[eid]
                    else:
                        self.target_edge_features = self.cache_edge_buffer[eid]

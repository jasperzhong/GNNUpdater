...
from gnnupdater.utils import get_batch_no_neg
...
class GNNLabStaticCache(Cache):
    ...
    def init_cache(self, *args, **kwargs):
        ...
        batch_size = kwargs.get('batch_size', 4000)
        ...
        presampling_overhead = time.time() - start
        self.node_cache_overhead += presampling_overhead
        self.edge_cache_overhead += presampling_overhead
        ...
        if self.distributed:
            ...
            if self.node_feats is not None:
                ...
                if (node_sampled_count[cache_node_id.cpu()] == 0).sum().item() > 0:

...
SamplingResult TemporalSampler::SampleLayer(
    ...
    uint32_t snapshot, bool sample_all) {
  // NB: it seems to be necessary to set the device again.
  CUDA_CALL(cudaSetDevice(device_));

  std::size_t num_root_nodes = dst_nodes.size();
  std::size_t maximum_sampled_nodes = 0;
  std::vector<uint32_t> offsets(num_root_nodes+1, 0);
  if (sample_all) {
    // sample all neighbors
    auto all_num_edges = graph_.out_degree(dst_nodes);
    std::size_t max_num = 0;
    for (int i = 0; i < num_root_nodes; i++) {
      offsets[i] = i == 0 ? 0 : offsets[i - 1] + all_num_edges[i - 1];
      maximum_sampled_nodes += all_num_edges[i];
      max_num = std::max(max_num, all_num_edges[i]);
    }
    offsets[num_root_nodes] = maximum_sampled_nodes;
  } else {
    maximum_sampled_nodes = fanouts_[layer] * num_root_nodes;
  }
  ...
  if (sample_all) {
    ...
    CUDA_CALL(cudaMalloc(&d_offsets, (num_root_nodes+1) * sizeof(uint32_t)));
    CUDA_CALL(cudaMemcpy(d_offsets, offsets.data(),
                         (num_root_nodes+1) * sizeof(uint32_t),
                         cudaMemcpyHostToDevice));
    uint32_t num_blocks = (num_root_nodes + num_threads_per_block - 1) /
                        num_threads_per_block;
    // std::cout << "num_root_nodes: " << num_root_nodes << "\tmax_sampled_nodes: " << maximum_sampled_nodes << std::endl;
    SampleLayerAllKernel<<<num_blocks, num_threads_per_block,
                          shared_memory_size_, stream_holders_[snapshot]>>>(
        graph_.get_device_node_table(), graph_.num_nodes(), prop_time_,
        d_root_nodes, d_root_timestamps, snapshot, num_snapshots_,
        snapshot_time_window_, maximum_sampled_nodes, d_offsets, num_root_nodes, d_src_nodes, d_eids,
        d_timestamps, d_delta_timestamps, d_num_sampled);
  } else if (sampling_policy_ == SamplingPolicy::kSamplingPolicyRecent) {
    ...
    if (layer == 0) {
      for (int snapshot = 0; snapshot < num_snapshots_; ++snapshot) {
        layer_results.push_back(
            SampleLayer(dst_nodes, dst_timestamps, layer, snapshot, sample_all));
      }
      ...
      for (int snapshot = 0; snapshot < num_snapshots_; ++snapshot) {
        ...
        layer_results.push_back(
            SampleLayer(all_nodes, all_timestamps, layer, snapshot, sample_all));
      }
      results.push_back(layer_results);
    }
  }

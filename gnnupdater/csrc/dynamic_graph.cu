...
std::vector<NIDType> DynamicGraph::get_all_neighbors(
    const std::vector<NIDType>& nodes, 
    const std::vector<int>& counts) const {
  std::vector<std::vector<NIDType>> all_neighbors(nodes.size());
  std::vector<std::future<void>> futures;
  std::mutex result_mutex;  // 用来保护并发写入
  ...
  for (size_t i = 0; i < nodes.size(); ++i) {
    futures.push_back(std::async(std::launch::async, [&, i]() {
      ...
      auto& list = h_copy_of_d_node_table_[nodes[i]];
      size_t total_size = 0;
      auto block = list.tail;
      ...
      dst_nodes.reserve(total_size);

      block = list.tail;
      while (block != nullptr) {
        dst_nodes.insert(dst_nodes.end(), block->dst_nodes, block->dst_nodes + block->size);
        block = block->prev;
      }

      // 使用 mutex 来保护写入 all_neighbors[i]
      std::lock_guard<std::mutex> lock(result_mutex);
      // copy the neighbors for `counts[i]` times
      for (size_t j = 0; j < counts[i]; ++j) {
        all_neighbors[i].insert(all_neighbors[i].end(), dst_nodes.begin(), dst_nodes.end());
      }
    }));
  }
  ...
  // flatten the result
  std::vector<NIDType> results;
  // check all_neighbors
  auto total_neighbors = 0;
  for (auto& neighbors : all_neighbors) {
    total_neighbors += neighbors.size();
  }
  results.reserve(total_neighbors);

  for (auto& neighbors : all_neighbors) {
    results.insert(results.end(), neighbors.begin(), neighbors.end());
  }

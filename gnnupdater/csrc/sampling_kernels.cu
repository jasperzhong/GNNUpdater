...
__global__ void SampleLayerAllKernel(
    const DoublyLinkedList* node_table, std::size_t num_nodes, bool prop_time,
    const NIDType* root_nodes, const TimestampType* root_timestamps,
    uint32_t snapshot_idx, uint32_t num_snapshots,
    TimestampType snapshot_time_window, uint32_t maximum_sampled_nodes,
    uint32_t* offsets, uint32_t num_root_nodes, NIDType* src_nodes, EIDType* eids, 
    TimestampType* timestamps, TimestampType* delta_timestamps, uint32_t* num_sampled) {
  uint32_t tid = threadIdx.x + blockIdx.x * blockDim.x;
  ...
  uint32_t max_edges = offsets[tid+1] - offset;
  uint32_t sampled_count = 0;

  // NB: the tail block is the newest block
  auto curr = node_table[nid].tail;

  while (curr != nullptr && sampled_count < max_edges) {
    auto curr_block = *curr;
    if (curr_block.capacity == 0) {
      // the block is empty
      curr = curr_block.prev;
    ...
    // Copy all edges in the time range
    for (int i = start_idx; i < end_idx && sampled_count < max_edges; ++i) {
      src_nodes[offset + sampled_count] = curr_block.dst_nodes[i];
      eids[offset + sampled_count] = curr_block.eids[i];
      auto timestamp = curr_block.timestamps[i];
      timestamps[offset + sampled_count] = prop_time ? root_timestamp : timestamp;
      delta_timestamps[offset + sampled_count] = root_timestamp - timestamp;
      sampled_count++;
    }
  ...
  // Fill remaining slots with kInvalidNID
  for (uint32_t i = sampled_count; i < max_edges; ++i) {
    src_nodes[offset + i] = kInvalidNID;
  }

  num_sampled[tid] = sampled_count;
}

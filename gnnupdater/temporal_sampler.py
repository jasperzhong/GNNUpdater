...
from libgnnupdater import SamplingPolicy, SamplingResult, _TemporalSampler
...
class TemporalSampler:
    ...
    def __init__(
        ...
        print("TemporalSampler: sample_strategy={}, fanouts={}, num_snapshots={}, snapshot_time_window={}, prop_time={}".format(
            sample_strategy, fanouts, num_snapshots, snapshot_time_window, prop_time))
    ...
    def sample(self, target_vertices: np.ndarray, timestamps: np.ndarray, sample_all=False) -> List[List[DGLBlock]]:
        """
        ...
        if self._is_static:
            sampling_results = self._sampler.sample(
                ...
                np.full(target_vertices.shape,
                        np.finfo(np.float32).max), sample_all)
        else:
            sampling_results = self._sampler.sample(
                target_vertices, timestamps, sample_all)
    ...
    def sample_layer(self, target_vertices:  np.ndarray, timestamps: np.ndarray,
                     layer: int, snapshot: int, to_dgl_block: bool = True, sample_all: bool = False) \
        ...
        """
        sampling_result = self._sampler.sample_layer(
            target_vertices, timestamps, layer, snapshot, sample_all)
        if to_dgl_block:
    ...
    def _to_dgl_block_layer_snapshot(self, sampling_result: SamplingResult) -> DGLBlock:
        src, dst = sampling_result.col(), sampling_result.row()

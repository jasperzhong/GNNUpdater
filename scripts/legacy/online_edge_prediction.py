...
import gnnupdater.cache as caches
...
from gnnupdater.utils import (DstRandEdgeSampler, EarlyStopMonitor,
...
parser.add_argument("--update-per-n-minute", type=int, default=1,
                    help="number of minutes for online learning")
parser.add_argument("--update-interval", type=int, default=10000,
                    help="number of samples before updating the model")
...
def split_chunks_by_days_gdelt(df: pd.DataFrame, minutes=1):
    df['time_minute'] = ((df['time'] * 15) // minutes).astype(int)
    grouped = df.groupby('time_minute')
    chunks = [group for _, group in grouped]
    return chunks


def split_chunks_by_days_netflix(df: pd.DataFrame, minutes=1):
    df['time_minute'] = (df['time'] / 60 // minutes).astype(int)
    grouped = df.groupby('time_minute')
    chunks = [group for _, group in grouped]
    return chunks


def split_chunks_by_days_reddit(df: pd.DataFrame, minutes=1):
    df['time_minute'] = (df['time'] / 60 // minutes).astype(int)
    grouped = df.groupby('time_minute')
    chunks = [group for _, group in grouped]
    return chunks
...
def split_chunks_by_update_interval(df: pd.DataFrame, update_interval: int):
    chunks = []
    start = 0
    while start < len(df):
        end = min(start + update_interval, len(df))
        chunks.append(df.iloc[start:end])
        start = end

    if len(chunks[-1]) < 100:
        # drop the last chunk if it is too small
        chunks.pop()
...
def evaluate(df, sampler, model, criterion, cache, device, rand_edge_sampler, restore_memory=False):
    model.eval()
    val_losses = list()
    aps = list()
    aucs_mrrs = list()

    if args.use_memory and restore_memory:
        # backup the memory
        if args.distributed:
            orig_mem = model.module.memory.backup()
        else:
            orig_mem = model.memory.backup()
    ...
    if args.use_memory and restore_memory:
        # restore the memory
        if args.distributed:
            model.module.memory.restore(orig_mem)
        else:
            model.memory.restore(orig_mem)
...
def main():
    ...
    data_config['mem_resource_type'] = 'cuda'
    ...
    chunks = split_chunks_by_update_interval(phase2_df, args.update_interval)
    # update_per_n_minutes = args.update_per_n_minute
    # if args.data == 'GDELT':
    #     chunks = split_chunks_by_days_gdelt(phase2_df, update_per_n_minutes)
    # elif args.data == 'NETFLIX':
    #     chunks = split_chunks_by_days_netflix(phase2_df, update_per_n_minutes)
    # elif args.data == 'REDDIT' or args.data == 'LASTFM' or args.data == 'MOOC' or args.data == 'WIKI':
    #     chunks = split_chunks_by_days_reddit(phase2_df, update_per_n_minutes)
    # else:
    #     raise NotImplementedError

    del phase2_df
    args.batch_size = 1000 # model_config['batch_size']
    ...
    if os.path.exists(checkpoint_path):
        ...
        train(phase1_train_df, phase1_val_df, sampler,
              model, optimizer, criterion, cache, device, train_rand_sampler, val_rand_sampler, checkpoint_path)
    ...
    val_ap_list = []
    val_auc_list = []
    incremental_df_len_list = []
    node_cache_hit_rate_list = []
    edge_cache_hit_rate_list = []
    ts_list_start = [chunk['time'].iloc[0] for chunk in chunks]
    ts_list_end = [chunk['time'].iloc[-1] for chunk in chunks]
    build_graph_time_list = []
    prepare_data_time_list = []
    training_time_list = []

    # load the best model
    checkpoint_path = os.path.join(get_project_root_dir(),
                                    'phase2_{}_{}_{}_{}_{}_{}_{}_{}.pt'.format(args.model, args.data, args.replay_ratio, args.epoch, args.val_ratio, args.update_interval, args.retrain, args.early_stop))
    for i, increment_df in enumerate(chunks):
        ...
        build_graph_time_list.append(phase2_build_graph_time)
        ...
        # NB: restoring the memory is necessary here otherwise the memory would leak information in the upcoming training
        ap, auc = evaluate(
            val_df, sampler, model, criterion, cache, device, val_rand_sampler, restore_memory=True)
        if args.distributed:
        ...
        incremental_df_len_list.append(len(increment_df))
        if args.rank == 0:
            logging.info("incremental step: {}".format(len(increment_df)))
            logging.info(
                "{}th incremental evalutae ap: {} auc: {}".format(i+1, ap, auc))

        phase2_train_start = time.time()
        if args.retrain:
            prepare_data_time = time.time()
            ...
            phase2_train_df = phase2_train_df.iloc[train_index].reset_index(drop=True)
            val_index = list(
                range(args.rank, len(phase2_val_df), args.world_size))
            phase2_val_df = phase2_val_df.iloc[val_index].reset_index(drop=True)

            prepare_data_time_list.append(time.time() - prepare_data_time)
            ...
            training_time_list.append(phase2_train_func_time)
    ...
    if args.rank == 0:
        ...
        # _{args.cache}_{args.edge_cache_ratio:.5f}_{args.node_cache_ratio:.5f}_reset{args.reset_cache}'
        prefix = f'{args.model}_{args.data}_{args.replay_ratio}_{args.epoch}_{args.val_ratio}_{args.update_interval}_{args.retrain}_{args.early_stop}'
        os.makedirs(subdir, exist_ok=True)
        df = pd.DataFrame({
            'ap': val_ap_list,
            'auc': val_auc_list,
            'len': incremental_df_len_list,
            'ts_start': ts_list_start,
            'ts_end': ts_list_end,
            'build_graph_time': build_graph_time_list,
            'prepare_data_time': prepare_data_time_list,
            'training_time': training_time_list
        })
        df.to_feather(os.path.join(subdir, f'{prefix}_res.feather'))
...
def train(train_df, val_df, sampler, model, optimizer, criterion,
          cache, device, train_rand_sampler, test_rand_sampler, checkpoint_path):
    best_ap = 0
    best_auc = 0
    best_e = 0
    ...
    for e in range(args.epoch):
            ...
            if (i+1) % args.print_freq == 0:
                ...
                if args.rank == 0:
                    logging.info('Epoch {:d}/{:d} | Iter {:d}/{:d} | Throughput {:.2f} samples/s | Loss {:.4f} | Cache node ratio {:.4f} | Cache edge ratio {:.4f} | Total sample time {:.2f}s'.format(e + 1, args.epoch, i + 1, int(len(
                        train_df)/args.batch_size), total_samples * args.world_size / (time.time() - epoch_time_start), total_loss / (i + 1), cache_node_ratio_sum / i, cache_edge_ratio_sum / i, total_sample_time))
        ...
        if args.rank == 0:
            logging.info("Epoch {:d}/{:d} | Validation ap {:.4f} | Validation auc {:.4f} | Train time {:.2f} s | Validation time {:.2f} s | Train Throughput {:.2f} samples/s | Cache node ratio {:.4f} | Cache edge ratio {:.4f} | Total sample time {:.2f}s".format(
                e + 1, args.epoch, val_ap, val_auc, epoch_time, val_time, total_samples * args.world_size / epoch_time, cache_node_ratio_sum / i, cache_edge_ratio_sum / i, total_sample_time))
        ...
        if early_stopper and early_stopper.early_stop_check(val_ap):

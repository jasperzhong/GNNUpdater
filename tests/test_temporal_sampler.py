...
from gnnupdater import DynamicGraph, TemporalSampler
...
class TestTemporalSampler(unittest.TestCase):
    ...
    @parameterized.expand(
        itertools.product(["cuda", "unified", "pinned", "shared"]))
    def test_sample_layer_all(self, mem_resource_type):
        # build the dynamic graph
        config = default_config.copy()
        config["mem_resource_type"] = mem_resource_type
        dgraph = DynamicGraph(**config)
        ...
        # sample 1-hop neighbors with sample_all=True
        sampler = TemporalSampler(dgraph, [2])
        target_vertices = np.array([0, 1, 2])
        block = sampler.sample_layer(target_vertices,
                                     np.array([2.5, 2.5, 2.5]), 0, 0,
                                     sample_all=True)

        # Check if all neighbors within the time range are sampled
        self.assertEqual(block.srcdata['ID'].tolist(), [
            0, 1, 2,
            1, 2, 3, 1, 2, 3, 1, 2, 3])
        self.assertEqual(block.srcdata['ts'].tolist(), [
        ...
        # Test with a narrower time window
        block = sampler.sample_layer(target_vertices,
                                     np.array([1.5, 1.5, 1.5]), 0, 0,
                                     sample_all=True)
        ...
        self.assertEqual(block.edata['ID'].tolist(), [0, 1, 3, 4, 6, 7])
        self.assertEqual(block.num_src_nodes(), 9)
        self.assertEqual(block.num_dst_nodes(), 3)
        self.assertEqual(block.edges()[0].tolist(), [3, 4, 5, 6, 7, 8])
        self.assertEqual(block.edges()[1].tolist(), [0, 0, 1, 1, 2, 2])

        print("Test sample_layer_all passed")

    @parameterized.expand(
        itertools.product(["cuda", "unified", "pinned", "shared"]))
    def test_sample_layer_all_vs_default(self, mem_resource_type):
        # build the dynamic graph
        config = default_config.copy()
        config["mem_resource_type"] = mem_resource_type
        dgraph = DynamicGraph(**config)
        ...
        sampler = TemporalSampler(dgraph, [2])
        target_vertices = np.array([0, 1, 2])
        timestamps = np.array([1.5, 1.5, 1.5])

        # Sample with default behavior (sample_all=False)
        block_default = sampler.sample_layer(target_vertices, timestamps, 0, 0)

        # Sample with sample_all=True
        block_all = sampler.sample_layer(
            target_vertices, timestamps, 0, 0, sample_all=True)

        # Check that sample_all=True returns at least as many neighbors as the default
        self.assertGreaterEqual(block_all.num_src_nodes(),
                                block_default.num_src_nodes())
        self.assertEqual(block_all.num_dst_nodes(),
                         block_default.num_dst_nodes())

        # Check that all edges in the default sample are also in the sample_all result
        default_edges = set(
            zip(block_default.edges()[0].tolist(), block_default.edges()[1].tolist()))
        all_edges = set(
            zip(block_all.edges()[0].tolist(), block_all.edges()[1].tolist()))
        self.assertTrue(default_edges.issubset(all_edges))

        print("Test sample_layer_all vs default passed")
